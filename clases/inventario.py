class Inventario:
    ...
    def generar_alerta(self,umbral_minimo):
        alerta=[prod.nombre for prod in self.lista_de_productos if prod.cantidad < umbral_minimo]
        return f'Productos por debajo del umbral: {', '.join(alerta)}' if alerta else 'No hay ningun producto por debajo del umbral determinado'